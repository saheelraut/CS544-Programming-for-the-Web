...
function setupRoutes(app) {
  ...
  app.get('/ss/:ssName',doDisplay(app));
  app.post('/ss/:ssName',doPostRequest(app))
  //must be last
  app.use(do404(app));
...
/** Index Page handler */
function doIndex(app) {
  return (async function (req, res) {
    try {
      const message = `Index Page`;
      res.send(app.locals.mustache.render('index',
          {index: [{msg: message,}]}));
    } catch (err) {
...
/** Spreadsheet handler */
function doDisplay(app) {
  return (async function (req, res) {
    try {
      const ssName = req.params.ssName;
      const spreadsheetData = await app.locals.store.readFormulas(ssName);
      const result = app.locals.mustache.render("sheet", {ssName: ssName, data: spreadsheetData});
      res.send(result);
    } catch (err) {
      const mapped = mapError(err);
      res.status(mapped.status).json(mapped);
...
function doPostRequest(app) {
  return (async function (req, res) {
    const ssName = req.params.ssName;
    const request = req.body;
    const error = {};
    try {
      if (validateUpdate(request, error)) {
        if (request.ssAct === "deleteCell") {
          const response = await app.locals.store.delete(ssName, request.cellId);
        } else if (request.ssAct === "updateCell") {
          const response = await app.locals.store.updateCell(ssName, request.cellId, request.formula);
        } else if (request.ssAct === "clear") {
          const response = await app.locals.store.clear(ssName);
        } else if (req.ssAct === "copyCell") {
          const response = await app.locals.store.updateCell(ssName, request.cellId, request.formula);
        }
        const spreadsheetData = await app.locals.store.readFormulas(ssName);
        const result = app.locals.mustache.render("sheet", {ssName: ssName, data: spreadsheetData});
...
const FIELDS =
    Object.keys(FIELD_INFOS).map((n) => Object.assign({name: n}, FIELD_INFOS[n]));
