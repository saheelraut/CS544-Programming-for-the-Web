...
export default class MemSpreadsheet {
   ...
   */
  query(cellId) {
    const result = this.getCell(cellId);  //return the cell object from the provided cellId
    if (result.id) {
      return {'formula': result.formula, 'value': result.value}
    } else {
  ...
  delete(cellId) {
    this._undos = {};
    const results = this._updateCell(cellId, cell => delete this._cells[cellId]);
    return results;
  }
  ...
  copy(destCellId, srcCellId) {
    ...
    const destFormula = srcAst.toString(destCallId);
    //@TODO
    return results;
  }
